...
s=2;
r=2;
echo -n "Test case 33: Sending 10 packets (sender number of 3,receiver number of 3) and expecting receiver to print them out in order: "
(for m in `seq 0 $s`;
do
    for n in `seq 0 $r`;
    do
...
sleep 5;
echo "exit";) | ./project3 -r 3 -s 3 > .output.33 2> .debug_output.33.log
