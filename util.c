...
int ll_isempty(LLnode * head){
    return ll_get_length(head);
}

void  ll_pop_nth_node(LLnode ** head_ptr,LLnode * pos)
{
    LLnode * pre;
    LLnode * cur;
...
LLnode * ll_get_nth_node(LLnode * head_ptr,int pos)
{
...
void ll_print_values(LLnode * head){
    LLnode * tmp;
    int count = 1;
    if (head == NULL)
        fprintf(stderr,"log: nothing!\n");
    else
    {
        fprintf(stderr,"log: %d: ",count);
        fprintf(stderr," %s\n",convert_char_to_frame(head->value)->data);
        tmp = head->next;
        while (tmp != head)
        {
            count++;
            fprintf(stderr,"log: %d: ",count);
            fprintf(stderr," %s\n",convert_char_to_frame(tmp->value)->data);
            tmp = tmp->next;
        }
//        return count;
    }
